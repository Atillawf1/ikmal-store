...
DB_NAME = "postgres"
DB_USER = "postgres"
DB_PASSWORD = "atillathegreat1453"  # Supabase şifreni buraya yaz
DB_PORT = "5432"
...
def odeme():
    ...
    # Verileri Supabase (Bulut) Veritabanına Kaydetme
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO siparisler (ad, kart_numarasi, SKT, cvv) VALUES (%s, %s, %s, %s)",
        (ad, kart_numarasi, skt, cvv)
    )
    conn.commit()
    cur.close()
    conn.close()
